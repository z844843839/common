...
public class CacheUtil {
    ...
    public static <T> T  get(String key, Class<T> c) {
        return key == null ? null : JSON.parseObject(JSON.toJSON(keyValueStorage.get(key)).toString(),c);
    }
    ...
    public static Boolean set(String key, Object value, Long time) {
        try {
            if (time > 0) {
               // JSONObject jSONObject=JSONObject.parseObject(JSONObject.toJSON(value).toString());
                keyValueStorage.setValAndTime(key,value, time);
            } else {
                set(key, value);
            }
            return true;
     ...
     */
    public static String createBusinessCode(String busCode,Integer strLength){
        //根据业务规则，获取当天六位数
        String dateStr = new SimpleDateFormat("YYMMdd")
                .format(new Date());
        //业务编码 + 6位日期 作为Key
    ...
    public static String increaseStr(String key,int strLength){
        ...
        String autoNumStr = String.format("%0" + strLength + "d", autoNum);
        //设置编码保存时长
        setExpireTime(key,365*2*24 * 60 * 60L);
        return autoNumStr;
    }
